...
suspend fun extractMovieAPIlinks(serverid: String, movieid: String, MOVIE_API: String): String {
  val link = app.get("$MOVIE_API/ajax/get_stream_link?id=$serverid&movie=$movieid").document.toString()
...
suspend fun decodesmashy(url: String): String {
  ...
  val string =
    doc.toString().substringAfter("#2").substringBefore("\"").replace(Regex("//.{16}"), "").let { DecodeBase64(it) }
